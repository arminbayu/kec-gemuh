          ...
          <div class="box">
            ...
            <div class="box-body">
                                    ...
                                    <span class="mailbox-attachment-size">
                                      ...
                                      <form>
                                        <div class="form-group">
                                          ...
                                          <input id="nama_file" type="text" name="nama_file" class="form-control" value="Pengiriman">
                                        </div>
                                        <div hidden>
                                            <input id="fileupload" type="file" name="files" data-url="<?php echo base_url('master_pengiriman/upload_image')?>" >
                                        </div>
                                      </form>
              ...
              <div id="div_table_output">
                ...
                <table id="table_master_pengiriman" class="table table-bordered table-striped ">
                  <thead>
                    <tr>
...
<script type="text/javascript">
 ...
 function fungsi_set_status(value) {
  ...
  function get_data_pengiriman(tgl){
    if(tgl == ''){
      tgl = '00/00/0000 - 31/12/9999';
    }

    var table_master_pengiriman = $('#table_master_pengiriman').DataTable({
      dom: 'Blfrtip',
      buttons: [
          'excel', 'csv'
      ],
      ...
      "serverSide": true,
      ajax : {
        url  : '<?php echo base_url('Master_pengiriman/get_data_pengiriman');?>',
        type : 'POST',
        data : {
          tgl : tgl
        },
        beforeSend:function() {
          $('#table_master_pengiriman').LoadingOverlay("show");
        },
        complete:function() {
          $("#table_master_pengiriman").LoadingOverlay("hide", true);
        },
      },
      columns: [
        { data: 'no' },
        { data: 'master_pengiriman' },
        { data: 'status' },
        // { data: 'created_by' },
        { data: 'created_on' },
        { data: 'aksi' },
  ...
  $('#datepicker').change(function () {
    $('#table_master_pengiriman').DataTable().destroy();
  ...
  function simpan_pengiriman() {
    $.ajax({
      url   : '<?php echo base_url('Master_pengiriman/simpan');?>',
      data  : {
      ...
      success : function (r) {
        if (r.success) {
          $('#table_master_pengiriman').DataTable().ajax.reload( null, false );
          bersihkan();
        }
      }
    });
  ...
  function edit_data(id) {
    ...
    $.ajax({
      url   : '<?php echo base_url('Master_pengiriman/get_data_by');?>',
      data  : {
        'id' : id,
      },
      type  : 'POST',
      ...
      success : function (r) {
        hide_show_div(1);
        $('[name=id_pengiriman]').val(r.master_pengiriman_id);
        $('[name=pengiriman]').val(r.master_pengiriman);
  ...
  function delete_pengiriman(id) {
    ...
    if (msg) {
      $.ajax({
        url   : '<?php echo base_url('Master_pengiriman/delete');?>',
        data  : {
          'id' : id,
        },
        type  : 'POST',
