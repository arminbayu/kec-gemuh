          ...
          <div class="box">
            ...
            <div class="box-body">
              <div class="col-sm-12 well">
                <div class="row">
                  <form id="form_jenis_po">
                    <div class="form-row">
                      <div class="col-md-4 mb-3">
                        <label>Nama jenis_po : </label>
                        <input type="text" class="form-control" id="jenis_po" name="jenis_po" autocomplete="on" required>
                        <input type="hidden" id="id_jenis_po" name="id_jenis_po">
                      </div>
                      <div class="col-md-4 mb-3">
                        <label>Status : </label>
                        <table>
                      ...
                      <div class="col-sm-12" style="margin-top:10px;">
                        <button type="button" id="tambah_jenis_po" class="btn btn-success" name="button">Tambah</button>
                      </div>
                  </form>
                </div>
              </div>
              ...
              <table id="table_jenis_po" class="table table-bordered table-striped ">
                <thead>
                  <tr>
...
<script type="text/javascript">
 ...
 function fungsi_set_status(value) {
  ...
  function get_data_jenis_po(tgl){
    if(tgl == ''){
      tgl = '00/00/0000 - 31/12/9999';
    }

    var table_jenis_po = $('#table_jenis_po').DataTable({
      dom: 'Blfrtip',
      buttons: [
          'excel', 'csv'
      ],
      ...
      "serverSide": true,
      ajax : {
        url  : '<?php echo base_url('jenis_po/get_data_jenis_po');?>',
        type : 'POST',
        data : {
          tgl : tgl
        },
        beforeSend:function() {
          $('#table_jenis_po').LoadingOverlay("show");
        },
        complete:function() {
          $("#table_jenis_po").LoadingOverlay("hide", true);
        },
      },
      columns: [
        { data: 'no' },
  ...
  $('#datepicker').change(function () {
    $('#table_jenis_po').DataTable().destroy();
    get_data_jenis_po($(this).val());
  });

  $('#tambah_jenis_po').click(function() {
    if ($('#jenis_po').val() == '') {
  ...
  function simpan_jenis_po() {
    $.ajax({
      url   : '<?php echo base_url('jenis_po/simpan');?>',
      data  : {
        'id_jenis_po'   : $('#id_jenis_po').val(),
      ...
      success : function (r) {
        if (r.success) {
          $('#table_jenis_po').DataTable().ajax.reload( null, false );
          $('#form_jenis_po').find('input').each(function() {
            $(this).val($(this).data('default'));
            $('#tambah_jenis_po').html('Tambah');
  ...
  function edit_data(id) {
    $.ajax({
      url   : '<?php echo base_url('jenis_po/get_data_by');?>',
      data  : {
        'id' : id,
      },
      type  : 'POST',
      dataType : 'jSON',
      success : function (r) {
        $('[name=id_jenis_po]').val(r.jenis_po_id);
        $('[name=jenis_po]').val(r.jenis_po);
        $('#tambah_jenis_po').html('Ubah');
        if (r.status == 1) {
          $("#toggle_status").prop("checked", true);
        }else {
  ...
  function delete_jenis_po(id) {
    var msg = confirm('Apakah anda yakin ? ');
    if (msg) {
      $.ajax({
        url   : '<?php echo base_url('jenis_po/delete');?>',
        data  : {
          'id' : id,
        },
        type  : 'POST',
