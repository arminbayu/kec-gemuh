        ...
        <div class="col-xs-12">
          <div class="box">
            ...
            <div class="box-body">
                  ...
                  <form id="form_base">
                    <div class="form-row">
                      <div class="col-md-4 mb-3">
                        <label>Nama base : </label>
                        <input type="text" class="form-control" id="base" name="base" autocomplete="on" required>
                        <input type="hidden" id="id_base" name="id_base">
                      </div>
                      <div class="col-md-4 mb-3">
                        <label>Status : </label>
                        <table>
                      ...
                      <div class="col-sm-12" style="margin-top:10px;">
                        <button type="button" id="tambah_base" class="btn btn-success" name="button">Tambah</button>
                      </div>
                  </form>
                  <!-- <form id="form_base" data-parsley-validate>
                    <div class="col-sm-6">
                      ...
                      <div class="form-group">
                        <label></label>
                        <button type="button" id="tambah_base" class="btn btn-info btn-sm" name="button">Tambah</button>
                      </div>
                    </div>
                  </form> -->
                </div>
              ...
              <table id="table_base" class="table table-bordered table-striped ">
                <thead>
                  <tr>
...
<script type="text/javascript">
 ...
 function fungsi_set_status(value) {
  ...
  function get_data_base(tgl){
    if(tgl == ''){
      tgl = '00/00/0000 - 31/12/9999';
    }

    var table_base = $('#table_base').DataTable({
      dom: 'Blfrtip',
      buttons: [
          'excel', 'csv'
      ],
      ...
      "serverSide": true,
      ajax : {
        url  : '<?php echo base_url('base/get_data_base');?>',
        type : 'POST',
        data : {
          tgl : tgl
        },
        beforeSend:function() {
          $('#table_base').LoadingOverlay("show");
        },
        complete:function() {
          $("#table_base").LoadingOverlay("hide", true);
        },
      },
      columns: [
        { data: 'no' },
  ...
  $('#datepicker').change(function () {
    $('#table_base').DataTable().destroy();
    get_data_base($(this).val());
  });

  $('#tambah_base').click(function() {
    if ($('#base').val() == '') {
  ...
  function simpan_base() {
    $.ajax({
      url   : '<?php echo base_url('base/simpan');?>',
      data  : {
        'id_base'   : $('#id_base').val(),
        'base'      : $('#base').val(),
        'status'          : $('#status').val(),
      ...
      success : function (r) {
        if (r.success) {
          $('#table_base').DataTable().ajax.reload( null, false );
          $('#form_base').find('input').each(function() {
            $(this).val($(this).data('default'));
            $('#tambah_base').html('Tambah');
  ...
  function edit_data(id) {
    $.ajax({
      url   : '<?php echo base_url('base/get_data_by');?>',
      data  : {
        'id' : id,
      },
      type  : 'POST',
      dataType : 'jSON',
      success : function (r) {
        $('[name=id_base]').val(r.base_id);
        $('[name=base]').val(r.base_name);
        $('#tambah_base').html('Ubah');
        if (r.status == 1) {
          $("#toggle_status").prop("checked", true);
        }else {
  ...
  function delete_base(id) {
    ...
    if (msg) {
      $.ajax({
        url   : '<?php echo base_url('base/delete');?>',
        data  : {
          'id' : id,
        },
        type  : 'POST',
